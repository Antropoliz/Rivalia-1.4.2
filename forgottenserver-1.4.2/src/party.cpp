...
bool Party::leaveParty(Player* player)
{
	...
	updateMembersPosition(player, true);
...
bool Party::joinParty(Player& player)
{
	...
	updateSharedExperience();
	updateMembersPosition(&player, false);
...
void Party::updateMembersPosition(Player* player, bool remove)
{
	...
	const PlayerVector members = getAllMembers();
	for (auto& partyMember : members)
	{
		if (partyMember != player)
		{
			// Delete/Add map mark for all party members
			partyMember->sendPartyMemberPosition(name, vocationId, position);

			// Delete/Add map mark of all party members to player
			player->sendPartyMemberPosition(partyMember->getName(), partyMember->getVocationId(), !remove ? partyMember->getPosition() : Position());
		}
	}
}